...
void motorPIDcontrol()
{
  ...
  int left = inital_speed + (int)PIDvalue;
  int right = inital_speed - (int)PIDvalue;

  
  
  //Serial.println(left);
  
  //Serial.println(right);Serial.println("_______");Serial.println("");
  
	analogWrite(ENA, constrain(inital_speed + (int)PIDvalue, min_speed,max_speed)); //ustawienie szybkosci LEWEGO kompletu silnikow w zakresie 0 do 255
	analogWrite(ENB, constrain( inital_speed - (int)PIDvalue, min_speed,max_speed));  //ustawienie szybkosci PRAWEGO kompletu silnikow w zakresie 0 do 255
...
{
	// obsluga wyjątków	
  if(stringThree == "00100")
  {
    servo.write(180); 
    return 0;
  }
  if(stringThree == "01100")
  {
    servo.write(90); 
    return -1;
  }
  ...
  servo.write(40); 
  int err = -4 * refl_reads[2] + -2 * refl_reads[3] + refl_reads[4] + 2 * refl_reads[5] + 4 * refl_reads[6];
  err = err / divide;
	return  err;
}
...
{  
  ...
  // pusty string i dzielenie przez 0
  stringThree = "";
  divide = 0;
  ...
  for (int i=2; i<7; i++)
  {
    ...
    stringThree = stringThree + refl_reads[i];	
  }
